			...
			<container>
				<div class="left">
					<ul>
						...
						<li>
							<h3>产品管理</h3>
							<div class="title">
								<p style="background : yellow;"><a>全部商品</a></p>
				...
				<div class="right">
					<table>
						...
						<c:forEach items="${allGoods }" var="goods" varStatus="i">
						<tr>
							<td>${i.count }</td>
							...
							<c:if test="${goods.del==false }">
								<td><button type="button" onclick="del(${goods.id})" style="width:60px; height:30px;">下架</button></td>
							</c:if>
						</tr>
						</c:forEach>
					</table>
					<div class="page">
						<a class="button" href="admin/allGoods.do?adminPageNumber=1&pageSize='${pageSize }'">第一页</a>
						<a class="button" href="admin/allGoods.do?adminPageNumber=${adminPageNumber-1 }&adminPageSize=${adminPageSize }">前一页</a>
						<a class="button">${adminPageNumber }</a>
						<a class="button" href="admin/allGoods.do?adminPageNumber=${adminPageNumber+1 }&adminPageSize=${adminPageSize }">后一页</a>
						<a class="button" href="admin/allGoods.do?adminPageNumber=${max }&adminPageSize=${adminPageSize }">最后页</a>
...
function add(id){
	$.ajax({
		url : 'admin/goodsAdd.do?id='+id,
		type : 'post',
		success : function(result){
			location.href = "/shop/admin/allGoods.do?adminPageNumber=${adminPageNumber }&adminPageSize=${adminPageSize }";
		},
		error : function(){
			alert("失败");
		},
